...
4. **Heydon Pickering, _Inclusive Design Patterns_ (2016).** Source for the "degrade gracefully when source fails" pattern. The skill applies progressive enhancement: query best source first, fall back to lower tiers when blocked.

5. **Bruce Schneier, _Beyond Fear_ (2003).** Source for the BYOK-MCP audit-log flagging discipline. Provenance matters; users have a right to know which data came from which provider.
