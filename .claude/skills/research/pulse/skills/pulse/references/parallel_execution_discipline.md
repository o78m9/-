...
The choice of 1 q/sec is the **defensible conservative lower bound** across the public APIs the skill uses. Higher rates work _sometimes_ but break unpredictably. Lower rates are wasteful.
...
When 3 consecutive failures fire across _any_ sources, halt. Likely root cause: network sandbox issue, harness misconfiguration, or simultaneous-outage event. Report what was collected and tell the user.
...
2. **IETF RFC 6585 — _Additional HTTP Status Codes_ (2012).** Source for the 429 ("Too Many Requests") + `Retry-After` header semantics. The RFC formalizes the server-side rate-limit signaling that Rule 5 (plan-tier detection) honors. https://datatracker.ietf.org/doc/html/rfc6585
