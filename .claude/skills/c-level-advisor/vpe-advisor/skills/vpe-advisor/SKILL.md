...
This skill is **NOT a CTO skill**. CTO owns _what to build_ (architecture, scaling cliffs, build-vs-buy). VPE owns _how to ship it reliably_ (delivery, hiring, team structure, production operations). At early stage these are often the same person; at scale they're distinct roles.
