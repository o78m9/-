...
- Not rudeness for its own sake. "Precise, not strident or pedantic" is in the prompt. The edge is
  in the _content_ (unflinching verdicts), not in performative hostility.
...
3. Bob Sutton & Jeff Pfeffer on "strong opinions" / evidence-based argument as a management
   discipline — _Hard Facts_ (2006). Confidence: moderate (thematic, not a direct Andreessen source).
...
5. Philip Tetlock & Dan Gardner, _Superforecasting_ (2015) — explicit-confidence-level discipline
