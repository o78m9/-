...
1. **Brian Kernighan & Rob Pike, _The Practice of Programming_ (Addison-Wesley, 1999)** — Chapter 1 on Style. The point that "names describe roles, not types" generalizes: a user's voice describes their habits, not their aspirations. Sample-based extraction captures habits.

2. **Steven Pinker, _The Sense of Style_ (Viking, 2014)** — Chapter on register and the "Classic Style" trap. Self-described voice often defaults to Classic Style ideals that the user's actual voice doesn't match.

3. **Bryan Garner, _Garner's Modern English Usage_ (5th ed., Oxford, 2022)** — Sections on register variation and register-adaptation across contexts. The justification for requiring sample variance (peer / external / transactional).

4. **Geoffrey Pullum, _The Cambridge Grammar of the English Language_ (Cambridge, 2002), Chapter 12** — Register theory. Establishes that register is detectable from text features (sentence length, pronoun choice, hedging frequency) more reliably than from speaker self-report.
...
6. **John Searle, _Speech Acts_ (Cambridge, 1969)** — Performative theory. Useful framing for the "hard rules" (S3.Q6) discipline: hard rules are performatives the user commits to; voice is descriptive.

7. **Email-writing style guides at scale: _The Yahoo! Style Guide_ (St. Martin's, 2010), _The Microsoft Manual of Style_ (4th ed.).** Real-world style guides establish that register depends heavily on recipient + context, not on a single "professional voice." Justifies asking for sample variance.
