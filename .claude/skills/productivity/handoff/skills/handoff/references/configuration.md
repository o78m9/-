...
- Anthropic, _Claude Code plugin configuration_ — `${CLAUDE_PLUGIN_ROOT}` and hook wiring.
- POSIX `mktemp(1)` — atomic temp-file creation semantics.
- Atlassian Confluence, _page lifecycle and retention_ — the precedent for mtime-guarded cleanup.
- Honeycomb, _Observability config patterns_ — defaults that are safe; opt-ins that are explicit.
