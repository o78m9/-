...
Hard cap at 5. Each entry: skill name + one-line _why this session needs it_.
...
- Q: Should the SessionStart hook surface the _latest_ handoff or _all_ handoffs from today?
...
- Matt Pocock, _Handoff skill_ (MIT) — the no-duplication discipline and the original four content rules.
- Karpathy, _Software 3.0 talk_ (2025) — context engineering: optimize for what the next agent _cannot_ recover.
- Anthropic, _Claude Code SessionStart hooks documentation_ — surfacing prior context as data, not instructions.
- Google SRE Workbook, _Postmortem culture_ — naming open decisions and their forcing constraints.
- DORA / Forsgren et al., _Accelerate_ — referencing artifacts rather than narrating work.
