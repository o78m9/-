...
2. Add a one-line comment above the marker explaining _why_ this is safe.
...
- David Dworken (Anthropic), _PreToolUse security reminder hook_ (MIT) — the precedent for inline rule whitelisting via comment markers.
- OWASP, _Cheat Sheet: Secrets Management_ — what counts as a secret in modern apps.
- NIST SP 800-53, _AC-3 Access Enforcement_ — least-privilege principles applied to documentation artifacts.
- GitGuardian, _State of Secrets Sprawl_ (2024) — the empirical taxonomy of secrets that leak into developer artifacts.
- Truffle Security, _TruffleHog detector list_ — the canonical reference for high-precision secret patterns.
- ICO (UK), _Personal data definition_ — what counts as PII under GDPR.
