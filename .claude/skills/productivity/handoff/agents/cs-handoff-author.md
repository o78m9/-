...
6. **Save to the configured location.** Read config via `scripts/config_loader.py`. If first-run setup hasn't completed, propose it once: _"Run setup now? (Y/n)"_. Do not silently pick a location.
...
For implicit signals, propose before running: _"Want me to write a handoff for the next session?"_ — never silently.
