...
**Time to fill out: ~20 minutes.** This is the single source of truth for the deal. Re-pricings or term changes create a _new_ intake — do not edit in place.
...
Check each that applies. Each flag justifies _some_ commercial flexibility but the discount scorer requires at least one for above-band discounts.
...
```json
{
  ...
  "term_redlines": ["uncapped indemnity", "MFN pricing"]
}
```
...
- [ ] Strategic flags reflect _committed_, not hoped-for, value.
