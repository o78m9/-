...
export class TestRailClient {
  ...
  async getProjects(): Promise<TestRailProject[]> {
    const result = await this.request<{ projects: TestRailProject[] }>('GET', 'get_projects')
  ...
  async getCases(
    ...
    const result = await this.request<{ cases: TestRailCase[] }>('GET', endpoint)
    return result.cases ?? (result as unknown as TestRailCase[])
  }

  async addCase(sectionId: number, payload: TestRailCasePayload): Promise<TestRailCase> {
    return this.request<TestRailCase>('POST', `add_case/${sectionId}`, payload)
  }

  async updateCase(caseId: number, payload: Partial<TestRailCasePayload>): Promise<TestRailCase> {
    return this.request<TestRailCase>('POST', `update_case/${caseId}`, payload)
  }

  async addRun(projectId: number, payload: TestRailRunPayload): Promise<TestRailRun> {
    return this.request<TestRailRun>('POST', `add_run/${projectId}`, payload)
  }
  ...
  async addResultForCase(
    ...
    return this.request<TestRailResult>('POST', `add_result_for_case/${runId}/${caseId}`, payload)
  }
  ...
  async getResultsForCase(
    ...
    const result = await this.request<{ results: TestRailResult[] }>('GET', endpoint)
