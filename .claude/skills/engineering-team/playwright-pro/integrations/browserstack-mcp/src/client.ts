...
export class BrowserStackClient {
  ...
  async getSession(sessionId: string): Promise<BrowserStackSession> {
    return this.request<BrowserStackSession>('GET', `/automate/sessions/${sessionId}.json`)
  }
  ...
  async updateSession(
    ...
    return this.request<BrowserStackSession>('PUT', `/automate/sessions/${sessionId}.json`, update)
  }
