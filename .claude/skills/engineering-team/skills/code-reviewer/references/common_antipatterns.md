...
**Note:** ASP.NET Core has no synchronization context, so `ConfigureAwait(false)` is _not required_ in ASP.NET Core application code — but it doesn't hurt and is mandatory for shared libraries.
