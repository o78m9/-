...
A common failure mode: users delegate curation to the LLM ("just ingest all my Pocket articles"). Don't. Curation is where human judgment lives. The LLM can help you decide _whether_ to read a source, but you pick what makes it into `raw/`.
...
- Ousterhout's _A Philosophy of Software Design_ — for why "deep modules" (well-summarized pages) beat shallow ones
